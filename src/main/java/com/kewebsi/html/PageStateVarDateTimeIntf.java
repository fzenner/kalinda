...
public interface PageStateVarDateTimeIntf extends PageStateVarIntf<LocalDateTime>  {
    ...
    /**
     * Contains the last successfully parsed date newValue or the last date newValue set via a LocalDate object.
     * If the effective newValue of the PageState variable (see {@link PageStateVarIntf#getValCore()}  ) is not null, this date buffer is null.
     * That is to make clear that the buffer should only be accessed when there is no real newValue.
     * The date buffer works similar to the unparsed string newValue (see {@link #getUnparsedStringValueDate()} )
     ...
     * that could be successfully parsed into a LocaDate. Returns also null, if the newValue of this PageStateVar is
     * non-null.
     *
     */
